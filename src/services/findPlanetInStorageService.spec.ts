...
import { findPlanetInStorage } from "./findPlanetInStorageService";
...
describe("Find Planet In Storage by ID", () => {
  ...
  it("should return the correct planet by id", () => {
    storage.set(
    ...
    const storagedPlanet = findPlanetInStorage(planetExample.id, storage);
    expect(storagedPlanet).not.toBeNull();
    expect(storagedPlanet?.data).toEqual(planetExample);
  });

  it("should return null if it finds no stored planet", () => {
    const storagedPlanet = findPlanetInStorage(planetExample.id, storage);
    expect(storagedPlanet).toBeNull();
  });
});
